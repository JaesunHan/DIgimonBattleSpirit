...
void gameOverScene::update()  
{
	...
	if (_cntIdleScore > 650)
	{
		...
		if (_pResult == WIN)
		{
			stageScene* tmp = (stageScene*)SCENEMANAGER->findScene("스테이지씬")->parent;
			tmp->init();
			//tmp->setCameraRatio();
...
void gameOverScene::draw()	 
{
	...
	*/
	...
	if (_idleScoreBoard)
	{
		...
		_pImg[DRAW]->aniRender(hdc, CAMERAMANAGER->getX() + 42, CAMERAMANAGER->getY() + 90, _pAnim[DRAW]);
		_eImg[DRAW]->aniRender(hdc, CAMERAMANAGER->getX() + 153, CAMERAMANAGER->getY() + 90, _eAnim[DRAW]);
	}
...
RESULT gameOverScene::drawjudgement(HDC hdc)
{
	
	//플레이어가 이김
	if (_peMarbles[0] > _peMarbles[1])
	{
		_pImg[WIN]->aniRender(hdc, CAMERAMANAGER->getX() + 42, CAMERAMANAGER->getY() + 90, _pAnim[WIN]);
		_eImg[LOSE]->aniRender(hdc, CAMERAMANAGER->getX() + 153, CAMERAMANAGER->getY() + 90, _eAnim[LOSE]);
		_pResult = WIN;
		_eResult = LOSE;
	}
	//비김
	...
	{
		_pImg[LOSE]->aniRender(hdc, CAMERAMANAGER->getX() + 42, CAMERAMANAGER->getY() + 90, _pAnim[LOSE]);
		_eImg[WIN]->aniRender(hdc, CAMERAMANAGER->getX() + 153, CAMERAMANAGER->getY() + 90, _eAnim[WIN]);
		_pResult = LOSE;
		_eResult = WIN;
...
void gameOverScene::setPEImageNAnim()
{
	...
	for (int i = 0; i < RESULT_END; ++i)
	{
		...
		wsprintf(pStr, "%s%s", _peName[0], _fileNameKey[i].c_str());
		...
		wsprintf(eStr, "%s%s", _peName[1], _fileNameKey[i].c_str());
